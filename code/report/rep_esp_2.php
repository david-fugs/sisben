...
$query = "SELECT 
        ...
        FROM 
            encVentanilla ev
        LEFT JOIN 
            comunas com ON ev.id_com = com.id_com 
        LEFT JOIN 
            barrios bar ON ev.id_bar = bar.id_bar 
        LEFT JOIN 
            corregimientos cor ON ev.id_correg = cor.id_correg 
        LEFT JOIN 
            veredas ver ON ev.id_vere = ver.id_vere 
        LEFT JOIN 
            usuarios usu ON ev.id_usu = usu.id_usu 
        LEFT JOIN 
            encuestadores enc ON usu.id_usu = enc.id_usu 
        LEFT JOIN 
            integVentanilla iv ON iv.id_encVenta = ev.id_encVenta  
        WHERE 
            ev.fec_reg_encVenta >= ? AND ev.fec_reg_encVenta <= ?
        GROUP BY 
            ev.id_encVenta 
        ORDER BY 
