...
<?php
	...
	$query = "SELECT * FROM encCampo INNER JOIN usuarios ON encCampo.id_usu=usuarios.id_usu INNER JOIN encuestadores ON usuarios.id_usu=encuestadores.id_usu WHERE (doc_encCampo LIKE '%".$doc_encCampo."%') AND (num_ficha_encCampo LIKE '%".$num_ficha_encCampo."%') AND encCampo.id_usu=$id_usu ORDER BY encCampo.fec_rea_encCampo ASC";
	...
	$consulta = "SELECT * FROM encCampo INNER JOIN usuarios ON encCampo.id_usu=usuarios.id_usu INNER JOIN encuestadores ON usuarios.id_usu=encuestadores.id_usu WHERE (doc_encCampo LIKE '%".$doc_encCampo."%') AND (num_ficha_encCampo LIKE '%".$num_ficha_encCampo."%') AND encCampo.id_usu=$id_usu ORDER BY encCampo.fec_rea_encCampo ASC LIMIT " .(($paginacion->get_page() - 1) * $resul_x_pagina). "," .$resul_x_pagina;
	...
	while($row = mysqli_fetch_array($result))
	{
		...
		echo '
				<tr>
					...
					<td data-label="F. PRE.">'.$row['fec_pre_encCampo'].'</td>
					<td data-label="F. REA.">'.$row['fec_rea_encCampo'].'</td>
					<td data-label="DOC. USU">'.$row['doc_encCampo'].'</td>
					<td data-label="NOMBRE">'.$row['nom_encCampo'].'</td>
					<td data-label="FICHA">'.$row['num_ficha_encCampo'].'</td>
					<td data-label="EDIT"><a href="showencCampo.php?id_encCampo='.$row['id_encCampo'].'" ><img src="../../img/search.png" width=28 height=28></a></td>
					<td data-label="EDITAR INTG."><a href="showencCampoFamily.php?id_encCampo='.$row['id_encCampo'].'" ><img src="../../img/family.png" width=28 height=28></a></td>
					<td data-label="AGREGAR INTG."><a href="addencCampoFamily.php?id_encCampo='.$row['id_encCampo'].'" ><img src="../../img/addpeople1.png" width=28 height=28></a></td>
					<td data-label="ELIMINAR"><a href="eliminarCampo.php?id_encCampo='.$row['id_encCampo'].'" onclick="return confirm(\'¿ESTÁS SEGURO DE ELIMINAR ESTE REGISTRO?\')"><img src="../../img/eliminar.png" width=28 height=28></a></td>
				</tr>';
	}
