...
try {
    ...
    if (isset($_GET['id_usu'])) {
        logError("Parámetro id_usu recibido: " . $_GET['id_usu']);
    }
    ...
    logError("Condiciones WHERE: " . $where);
    ...
    logError("SQL Query: " . $sql_totales);
    
    $res = mysqli_query($mysqli, $sql_totales);
    
    // Verificar si la consulta se ejecutó correctamente
    if ($res === false) {
        logError("Error en la consulta SQL: " . mysqli_error($mysqli));
        echo "Error en la consulta: " . mysqli_error($mysqli);
        exit;
    }
    
    $num_rows = mysqli_num_rows($res);
    logError("Consulta ejecutada correctamente. Número de filas: " . $num_rows);

    // Aplicar color de fondo a las celdas A1 a Q1
    $sheet->getStyle('A1:Q1')->applyFromArray([
        'fill' => [
            'fillType' => \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID,
            'startColor' => [
                'rgb' => 'ffd880', // Color deseado en formato RGB
            ],
        ],
    ]);
    ...
    $sheet->getStyle('A1:Q1')->applyFromArray(['font' => $styleHeader['font'], 'fill' => $styleHeader['fill'], 'alignment' => $styleHeader['alignment']]);
    ...
    while ($row = mysqli_fetch_array($res, MYSQLI_ASSOC)) {
        ...
        $sheet->getStyle('A' . $rowIndex . ':Q' . $rowIndex)->applyFromArray(['font' => $boldFontStyle]);
        $rowIndex++;
        $rowCount++;
    }
    
    logError("Filas procesadas: " . $rowCount);

    // Nombre del archivo con la fecha actual
    $nombreArchivo = 'Encuestas_' . $fecha_inicio . '_' . $fecha_fin . '.xlsx';
    $writer = new Xlsx($spreadsheet);
    
    logError("Preparando descarga del archivo: " . $nombreArchivo);
    ...
    logError("Archivo descargado exitosamente");
    ...
    echo "<br><br>Detalles técnicos registrados en debug.log";
    exit;
} catch (Error $e) {
