...
if (!function_exists('limpiarTexto')) {
    ...
    while ($row = mysqli_fetch_array($res_movimientos, MYSQLI_ASSOC)) {
        ...
        if ($integ_mov) {
            $sheet3->setCellValue('B' . $rowIndex3, $integ_mov['doc_encVenta'] ?? $row['doc_encVenta']);
            ...
            $sheet3->setCellValue('G' . $rowIndex3, $row['nom_encVenta'] ?? '');
            ...
            $sheet3->setCellValue('M' . $rowIndex3, $row['tipo_movimiento'] ?? '');
            $sheet3->setCellValue('N' . $rowIndex3, $integ_mov['cant_integMovIndep'] ?? $row['integra_encVenta']);
